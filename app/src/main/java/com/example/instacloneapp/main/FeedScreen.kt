...
fun FeedScreen(navController: NavController, vm: IgViewModel) {
    ...
    Column(
        ...
        Row(
            ...
            Button(onClick = { navigateTo(navController, DestinationScreen.Text ) }) {
                Text(text = "Chat")
            }
        }
        PostsList(
...
fun PostsList(
    ...
    Box(modifier = modifier) {
        LazyColumn {
            items(items = posts) {
                Post(it, currentUserId, vm) {
                    navController.navigate(DestinationScreen.SinglePost.route+"/${it.postId}")
                }
            }
        }
